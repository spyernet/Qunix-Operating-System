...
pub extern "C" fn _start(argc: u64, argv: *const *const u8) -> ! {
    ...
    while i<a.len(){match a[i].as_str(){"--"=>{i+=1;break;},s if !s.starts_with('-')=>files.push(a[i].clone()),_=>{}}i+=1;}
    ...
    let process=|fd:i32|{
        ...
        for line in s.split('\n'){
            ...
            wstr(&rev); write(STDOUT,b"\n");
        }
    };
    if files.is_empty(){process(STDIN);}
    else{for f in &files{if f=="-"{process(STDIN);}else{
        let mut p=f.clone();p.push('\0');let fd=open(p.as_bytes(),O_RDONLY,0);if fd<0{continue;}process(fd as i32);close(fd as i32);
    }}}
    exit(0)
}
