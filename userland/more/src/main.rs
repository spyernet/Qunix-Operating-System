    ...
    let pager=|fd:i32|{
        ...
        while row<lines.len(){
            ...
            if row<lines.len(){
                ...
                write(STDERR,b"\r        \r");
                if n>0&&(b[0]==b'q'||b[0]==3){break;}
            }
        }
    };
    if files.is_empty(){pager(STDIN);}
    else{for f in &files{if f=="-"{pager(STDIN);}else{let mut p=f.clone();p.push('\0');let fd=open(p.as_bytes(),O_RDONLY,0);if fd<0{continue;}pager(fd as i32);close(fd as i32);}}}
    exit(0)
}
