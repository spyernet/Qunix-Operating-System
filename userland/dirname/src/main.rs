...
pub extern "C" fn _start(argc: u64, argv: *const *const u8) -> ! {
    ...
    let mut zero = false; let mut i = 1;
    while i < a.len() {
        match a[i].as_str() {
            "-z"|"--zero"     => zero=true,
            "--help"          => { w("Usage: dirname [-z] NAME...\nStrip last component from file name.\n"); exit(0); }
            "--version"       => { w("dirname (Qunix) 1.0\n"); exit(0); }
            "--"              => { i+=1; break; }
            _                 => break,
        }
        i+=1;
    }
    if i >= a.len() { e("dirname: missing operand\n"); exit(1); }
    let sep: &[u8] = if zero { b"\0" } else { b"\n" };
    for path in &a[i..] {
        let trimmed = path.trim_end_matches('/');
        let dir = if let Some(p) = trimmed.rfind('/') {
            if p == 0 { "/" } else { &trimmed[..p] }
        } else { "." };
        w(dir); write(STDOUT, sep);
    }
    exit(0)
}
