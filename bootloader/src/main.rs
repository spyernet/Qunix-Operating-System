...
fn efi_main(image: Handle, mut st: SystemTable<Boot>) -> Status {
    ...
    let fb_info = init_gop(bt, image);
    let kdata   = read_file(bt, image, "\\EFI\\QUNIX\\KERNEL.ELF");
    let init    = read_optional_file(bt, image, "\\EFI\\QUNIX\\INIT.ELF");
    let qshell  = read_optional_file(bt, image, "\\EFI\\QUNIX\\QSHELL.ELF");
    ...
    unsafe {
        ...
        for desc in mmap_iter {
            let dst = (mmap_store as *mut u8).add(count as usize * mmap_size.entry_size) as *mut MemoryDescriptor;
...
fn init_gop(bt: &BootServices, image: Handle) -> (u64, u32, u32, u32, u32) {
    let handles = bt
...
fn read_file(bt: &BootServices, image: Handle, path_str: &str) -> Vec<u8> {
    let handles = bt
...
fn read_optional_file(bt: &BootServices, image: Handle, path_str: &str) -> Option<Vec<u8>> {
    let handles = bt
        .locate_handle_buffer(SearchType::ByProtocol(&SimpleFileSystem::GUID))
        .ok()?;
    let handle = *handles.handles().first().unwrap();
